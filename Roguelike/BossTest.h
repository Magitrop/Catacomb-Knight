...
public:
	...
	virtual void Initialize(int withID) override
	{
		...
		maxActionsCount = 2;
		maxHealth = curHealth = 3000; //55 + Controller.sessionDay * 55 + (Controller.sessionDay / 2) * 55;
	...
	virtual void Update(void) override
	{
		if (map->tilemap->GetTile(x, y)->isExplored &&
			...
			PlayAnimations();
		Creature::Update();
		if (UpdateHealthbar(&curHealthInHealthbar, maxHealth * 0.5) && isAlive && healthbarFullPosition.w < 5)
		{
			actionsLeft = 0;
			curHealth = 0;
			isAlive = false;
