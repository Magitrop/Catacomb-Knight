...
class MapController;
class CreatureBlackSpiderling : public Creature
{
private:
	int curPathIndex = 0;
	List<Tile*>* curPath;
...
public:
	virtual ~CreatureBlackSpiderling() override
	{
		/*if (curPath != nullptr)
			delete curPath;*/
		delete idleAnim;
	...
	virtual void Initialize(int withID) override
	{
		...
		maxActionsCount = 1;
		maxHealth = curHealth = 15 + Controller.sessionDay * 10 + (Controller.sessionDay / 2) * 10;
		dropShardsCount = 2 + Controller.sessionDay * 2;
		viewDistance = 6;
		curHealthInHealthbar = curHealth;
		damage = 5 + Controller.sessionDay * 5 + (Controller.sessionDay / 2) * 5;
		...
		objectTexture = SDL_CreateTextureFromSurface(Initializer.renderer, surf = IMG_Load("textures/black_spiderling.png"));
		SDL_FreeSurface(surf);
	}
