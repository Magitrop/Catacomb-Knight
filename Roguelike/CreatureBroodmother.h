...
class MapController;
class CreatureBroodmother : public Creature
{
private:
	int curPathIndex = 0;
	List<Tile*>* curPath;
...
public:
	virtual ~CreatureBroodmother() override
	{
		/*if (curPath != nullptr)
			delete curPath;*/
		delete idleAnim;
	...
	virtual void Initialize(int withID) override
	{
		...
		maxActionsCount = 3;
		maxHealth = curHealth = 200 + Controller.sessionDay * 80 + (Controller.sessionDay / 2) * 80;
		dropShardsCount = 20 + rand() % 15 + Controller.sessionDay * 6;
		viewDistance = 7;
		...
		objectTexture = SDL_CreateTextureFromSurface(Initializer.renderer, surf = IMG_Load("textures/broodmother.png"));
		SDL_FreeSurface(surf);
	}
					...
					if (curPath != nullptr)
					{
						...
						else
						{
							...
							else
							{
								...
								int spiderlingsCount = 2;
								Creature* newCreature;
								for (int j = 0; j < spiderlingsCount && j < tilesToSpawn.size(); j++)
								{
									tile = tilesToSpawn[j];
									newCreature = new CreatureSpiderling();
