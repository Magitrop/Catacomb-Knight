...
class MapController;
class CreatureEvilEye : public Creature
{
private:
	int curPathIndex = 0;
	List<Tile*>* curPath;
...
public:
	virtual ~CreatureEvilEye() override
	{
		/*if (curPath != nullptr)
			delete curPath;*/
		delete idleAnim;
	...
	virtual void Initialize(int withID) override
	{
		...
		maxActionsCount = 3;
		maxHealth = curHealth = 80 + Controller.sessionDay * 70 + (Controller.sessionDay / 2) * 70;
		dropShardsCount = 12 + rand() % 15 + Controller.sessionDay * 5;
		viewDistance = 7;
		curHealthInHealthbar = curHealth;
		damage = 20 + Controller.sessionDay * 15 + (Controller.sessionDay / 2) * 15;
		...
		objectTexture = SDL_CreateTextureFromSurface(Initializer.renderer, surf = IMG_Load("textures/evil_eye.png"));
		SDL_FreeSurface(surf);
	}
